...
def _execute_or_fail(dbms: DBMS, command: str) -> QueryResult:
...
def _drop_table_if_exists(dbms: DBMS, table_name: str) -> None:
    dbms.execute(f"drop table {table_name}")
...
def _load_table(
    ...
    _drop_table_if_exists(dbms, table_name)
    _execute_or_fail(
        dbms,
...
def _measure_query(
    ...
    for _ in range(repeats):
        start = perf_counter()
        last_result = _execute_or_fail(dbms, command)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run an in-process memdb index benchmark."
    )
    ...
    dbms = DBMS(storage=create_storage(config.storage), query_factory=QueryFactory())
    dbms.init()

    target_id = args.rows // 2
    print("initialized in-process memdb benchmark")
    print(f"loading {args.rows} rows into {args.indexed_table} with index(id)")
    indexed_insert_seconds = _load_table(
        dbms, args.indexed_table, args.rows, with_index=True
    )
    print(
    ...
    plain_insert_seconds = _load_table(
        dbms, args.plain_table, args.rows, with_index=False
    )
    print(
